...
import { fetchGbpReviews, GbpNotConnectedError } from "@/lib/gbp-client";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    if (!token?.locationId) return NextResponse.json({ reviews: [] });
    return NextResponse.json({ reviews: await fetchGbpReviews(token.locationId) });
  } catch (err) {
